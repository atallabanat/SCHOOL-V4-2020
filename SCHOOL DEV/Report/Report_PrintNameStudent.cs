...
{
    public partial class Report_PrintNameStudent : Form
    {
        static string constring = ConfigurationManager.ConnectionStrings["Con"].ConnectionString;
        SqlConnection con = new SqlConnection(constring);
        public Report_PrintNameStudent()
        {
            InitializeComponent();
        }

        private void Report_PrintNameStudent_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                cmd.Parameters.AddWithValue("@ID", PrintNameStudent.printNameStudent.comboBox1.SelectedValue);
                cmd.Parameters.AddWithValue("@YaerSemesterID", Program.ID_Year);

                SqlDataAdapter da = new SqlDataAdapter(cmd);
                da.Fill(m, m.Tables[0].TableName);

                ReportDataSource reportDataSource = new ReportDataSource("DS_PrintNameStudent", m.Tables[0]);
