...
{
    public partial class Report_PrintClassPaid : Form
    {
        static string constring = ConfigurationManager.ConnectionStrings["Con"].ConnectionString;
        SqlConnection con = new SqlConnection(constring);
        public Report_PrintClassPaid()
        {
            InitializeComponent();
        }

        private void Report_PrintClassPaid_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                cmd.Parameters.AddWithValue("@IDClass", PrintClassPaid.printClassPaid.comboBox1.SelectedValue);
                cmd.Parameters.AddWithValue("@YaerSemesterID", Program.ID_Year);

                SqlDataAdapter da = new SqlDataAdapter(cmd);
                da.Fill(m, m.Tables[0].TableName);

                ReportDataSource reportDataSource = new ReportDataSource("DS_PrintClassPaid", m.Tables[0]);
