...
{
    public partial class Report_PrintClassStudent : Form
    {
        static string constring = ConfigurationManager.ConnectionStrings["Con"].ConnectionString;
        SqlConnection con = new SqlConnection(constring);
        public Report_PrintClassStudent()
        {
            InitializeComponent();
        }

        private void Report_PrintClassStudent_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                cmd.Parameters.AddWithValue("@IDClass", PrintClassStudent.printClassStudent.comboBox1.SelectedValue);
                cmd.Parameters.AddWithValue("@YaerSemesterID", Program.ID_Year);

                SqlDataAdapter da = new SqlDataAdapter(cmd);
                da.Fill(m, m.Tables[0].TableName);

                ReportDataSource reportDataSource = new ReportDataSource("DS_PrintClassStudent", m.Tables[0]);
